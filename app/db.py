...
async def set_active_robot_count(target_count: int) -> dict[str, int]:
    """Set the exact number of active robots.

    If target_count is higher than the current active count, inactive robots are
    reactivated first, then new robot records are created. If target_count is
    lower, extra active robots are marked as inactive. Existing confirmed
    bookings are kept intact; availability is recalculated from the new count.
    """
    ...
    async with get_db() as db:
        ...
        if target_count > active_count:
            ...
            deactivate = active_count - target_count
            cursor = await db.execute(
                "SELECT id FROM robots WHERE status='active' ORDER BY id DESC LIMIT ?",
                (deactivate,),
            )
            rows = await cursor.fetchall()
            for row in rows:
                await db.execute("UPDATE robots SET status='inactive' WHERE id=?", (row["id"],))
...
async def add_robot() -> int:
    async with get_db() as db:
        cursor = await db.execute("SELECT id FROM robots WHERE status='inactive' ORDER BY id LIMIT 1")
        row = await cursor.fetchone()
        if row:
            await db.execute("UPDATE robots SET status='active' WHERE id=?", (row["id"],))
            return int(row["id"])
