    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text="✅ -1 с обслуживания", callback_data="robots:maintenance_remove"),
            ],
            [InlineKeyboardButton(text="✏️ Установить всего роботов", callback_data="robots:set_total")],
            [InlineKeyboardButton(text="🔄 Обновить", callback_data="robots:refresh")],
        ]
    )


def support_admin_keyboard(client_telegram_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="💬 Ответить клиенту",
                    callback_data=f"support:reply:{client_telegram_id}",
                )
            ]
        ]
    )
...
def support_again_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✉️ Написать ещё", callback_data="support:start")]
        ]
    )
