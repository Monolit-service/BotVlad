...
from app.states import BookingForm, RobotSettings
...
async def robots_panel_text() -> str:
    ...
    return (
        ...
        f"Всего в базе: {summary.get('total', 0)}\n\n"
        "Активные роботы участвуют в расчёте свободных дат. "
        "Дата становится красной, когда свободных активных роботов не осталось.\n\n"
        "Можно менять число кнопками +1/-1 или установить точное количество."
    )
...
async def cb_robots(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if action == "add":
        await db.add_robot()
        await callback.answer("Добавлен 1 активный робот")
    elif action == "remove":
        ok = await db.remove_one_active_robot()
        await callback.answer("Убран 1 активный робот" if ok else "Нет активных роботов", show_alert=not ok)
    elif action == "set":
        await state.set_state(RobotSettings.count)
        if callback.message:
            await callback.message.answer(
                "Введите точное количество активных роботов числом.\n\n"
                "Например: 3\n"
                "Можно указать 0, если временно нет доступных роботов."
            )
        await callback.answer()
        return
    elif action == "refresh":
...
async def set_robot_count(message: Message, state: FSMContext) -> None:
    ...
    await db.set_active_robot_count(target_count)
    await state.clear()
    await message.answer(
        f"✅ Количество активных роботов установлено: {target_count}",
